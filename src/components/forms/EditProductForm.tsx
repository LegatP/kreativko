...
export default function EditProductForm({
  ...
  const [formData, setFormData] = useState({
    ...
    prompt: "",
    promptType: "create" as "create" | "edit",
  });
        ...
        if (productId) {
          ...
          if (product) {
            setFormData({
              slug: product.slug,
              name: product.name,
              defaultShirtColor: product.defaultShirtColor,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name.trim() || !formData.prompt.trim() || !productId) return;
    ...
    try {
      await updateProduct(productId, {
        ...
        designs: designs.filter((d) => d.url.trim()),
        categoryIds:
  ...
  const handleClose = () => {
    // Reset will happen on next open via useEffect
    onClose();
  };
                  ...
                  <div className="space-y-3">
                    {variables.map((variable, index) => (
                      ...
                      >
                        ...
                        <span>{variable.suggestions?.join(", ") || ""}</span>
                        <Input
                          size="sm"
